...
independent of both $p$ and $q$, so the statistic
$(N_{S,C}, N_{\neg S, C})$ is a sufficient statistic (so together
$N_{S,C}$ and $N_{\neg S, C}$ are jointly sufficient statistics).
...
The posterior distribution obtained is valid, from below. My
distribution for L comes from the fact it is proportional to a student's
t distribution (part a), which let me normalise it.

$$
\begin{aligned}
P(\Delta, \sigma, \beta | L_{1:M}, K) &\propto \pi(\Delta, \sigma, \beta) P(L_{1:M} | \Delta, \sigma, \beta) \\
\\
P(L | \Delta, \sigma, \beta) &= \frac{\Gamma(\beta)}{\Gamma(\beta - \frac{1}{2}) \sqrt{\pi \sigma^2}} \cdot \left( 1 + \frac{(L - (L^* + \Delta))^2}{\sigma^2} \right)^{-\beta} \\
\\
\text{So } \quad P(L_{1:M} | \Delta, \sigma, \beta) &= \left( \frac{\Gamma(\beta)}{\Gamma(\beta - \frac{1}{2}) \sqrt{\pi \sigma^2}} \right)^M \cdot \prod_{i=1}^M \left( 1 + \frac{(L_i - (L^* + \Delta))^2}{\sigma^2} \right)^{-\beta} \\
\\
\text{So } \quad \pi(\Delta, \sigma, \beta | L_{1:M}, K) &\propto \Theta(\sigma) \Theta(\beta - 2) \cdot \left( \frac{\Gamma(\beta)}{\Gamma(\beta - \frac{1}{2}) \sqrt{\pi \sigma^2}} \right)^M \cdot \prod_{i=1}^M \left( 1 + \frac{(L_i - (L^* + \Delta))^2}{\sigma^2} \right)^{-\beta}
\end{aligned}
$$
...
$$
\int_{-\infty}^{\infty} d\Delta \int_{0}^{\infty} d\sigma \int_{2}^{\infty} d\beta \left( \frac{\Gamma(\beta)}{\Gamma(\beta - \frac{1}{2}) \sqrt{\pi \sigma^2}} \right)^M \cdot \prod_{i=1}^M \left( 1 + \frac{(L_i - (L^* + \Delta))^2}{\sigma^2} \right)^{-\beta}
$$

This integral is normaliseable, as the integrand decays fast with high
$\beta, \Delta$ or $\sigma$.

### Part d

I used metrop to sample from the posterior distribution of the parameters. @fig-2 shows the joint distribution of $\delta$ and $\beta$, and @fig-3 and @fig-4 show the marginal distributions of all the parameters. The marginal distribution of $\delta$ suggests there is evidence for systematic bias in the measurements; there is very little density around 0 in its distribution. Additionally, the distribution of $\beta$, suggests there is evidence for a heavy tailed distribution; $\beta$ seems to have a lot of its probability mass around lower values, which corresponds to heavier tails. 

Hmm but maybe I should be considering beta and delta together as for there to be systematic bias AND heavy tails suggests there's a problem, if it's just one or the other it might be ok
...
calib_dat <- read.csv('calibration.csv', header=FALSE)
calib <- calib_dat[[1]]

log_posterior_3d <- function(theta){
  delta <- theta[1]
  sigma <- theta[2]
  beta <- theta[3]
  ...
  bigsum <- 0 
  for (i in 1:40){
    bigsum <- bigsum -beta*log(1+(calib[i]-(3.44+delta))^2/sigma^2)
  }
  
  log_post <- -2*log(beta) + 40*log(gamma(beta)) - 40*log(sqrt(pi*sigma^2)*gamma(beta-0.5)) + bigsum
  
  return (log_post)
}

nsmp <- 100000
params_initial_3d <- c(0.5, 0.5, 5) 

posterior_3d <- metrop(
  obj= log_posterior_3d,
  initial = params_initial_3d,
  nbatch=nsmp
)

posterior_samples3d <- as.matrix(posterior_3d$batch)
colnames(posterior_samples3d) <- c("delta", "sigma", "beta")

# Posterior samples
delta_samples <- posterior_samples3d[, "delta"]
sigma_samples <- posterior_samples3d[, "sigma"]
beta_samples  <- posterior_samples3d[, "beta"]

nsmp_burnin <- 1000

delta_samples <- delta_samples[nsmp_burnin:nsmp]
sigma_samples <- sigma_samples[nsmp_burnin:nsmp]
beta_samples <- beta_samples[nsmp_burnin:nsmp]
```

```{r}
#| echo: false
#| label: fig-2
#| fig-cap: "Joint Posterior density for beta and delta"

#going to figure out how to make this a contour plot
#Joint posterior in (delta, beta) as a scatter plot
plot(x = delta_samples, y = beta_samples, xlim = range(delta_samples), ylim = range(beta_samples), 
  xlab = "delta", ylab = "beta", pch='.')
```

```{r}
#| echo: false
#| label: fig-3
#| fig-cap: "Marginal Posterior Densities for delta and beta"


par(mfrow=c(1,2))

nbin <- 80
breaks1 <- seq(from = min(delta_samples), to = max(delta_samples), length.out = nbin)
hist(delta_samples, xlim = range(delta_samples), breaks = breaks1, main = "", freq = FALSE,
  col = "white",
  xlab = "delta", ylab = "posterior density")

breaks2 <- seq(from = min(beta_samples), to = max(beta_samples), length.out = nbin)
hist(beta_samples, xlim=range(beta_samples), breaks=breaks2, main="", freq=FALSE,
...
#| echo: false
#| label: fig-4
#| fig-cap: "Marginal Posterior Density for Sigma"

breaks3 <- seq(from = min(sigma_samples), to = max(sigma_samples), length.out = nbin)
hist(sigma_samples, xlim=range(sigma_samples), breaks=breaks3, main="", freq=FALSE,
...
formula <- "capacity~log(age)+log(weight)+cigarettes"
smoking_dat$weights <- 1/ (0.3^2)

fit <- glm(formula, data=smoking_dat, weights= weights)

# Extract the fit and covariance.

param_est <- as.numeric(fit$coefficients)
param_covar <- summary(fit)$cov.unscaled

# Print out the parameter estimates and the covariance matrix (implicitly
# assuming uniform improper priors).
