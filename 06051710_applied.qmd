...
survey participants are related to one another in some ways (at its
worst there could be direct relatives). This would mean due to shared
...
sampling. For example, sometimes when filling a form online the website
could glitch and record one participant's responses twice. These two
rows are therefore clearly not independent as if one of the rows records
a participant with cancer, their repeated response would also record a
...
statistic $\mathbf{\theta} = (N_{S,C}, N_{\neg S, C})$ is a sufficient
...
smoker (I sampled from the joint distribution using Markov Chain Monte
Carlo techniques). I also calculated the posterior probability ratio for
when p\>q versus when it is less than or equal to q. This reduced to the
Bayes factor, as the uniform distribution for p and q meant the prior
probability ratio was equal to 1. The Bayes factor was... which
demonstrated high evidence for...
